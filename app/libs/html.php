<?php
	...
	*/
	class Html {
		...
		public static function breadcrumbs($acciones,$textos,$parametros = false){
			if($parametros){
				echo "<script language='JavaScript'>breadcrumbParameters('".$acciones."','".$textos."','".$parametros."'); document.location.href = '#top';</script>";
			}
			else{
				echo"<script language='JavaScript'>breadcrumb('".$acciones."','".$textos."'); document.location.href = '#top';</script>";
			}
		} 
		...
		public static function linkAjax($accion, $text, $contenedor) {
			...
			$params["class"] = isset($params["class"]) ? $params["class"]." jsRemote" : "jsRemote";
			$params["id"] = isset($params["id"]) ? $params["id"] : "jsRemote";
			return  link_to($params);
		}
		...
		public static function linkAjaxConfirmado($accion, $text, $contenedor){
            ...
            $params["rel"] = "#" . $contenedor;
			$params["class"] = isset($params["class"]) ? $params["class"]." jsRemoteEliminar" : "jsRemoteEliminar";
			$params["id"] = isset($params["id"]) ? $params["id"] : "jsRemoteEliminar";
